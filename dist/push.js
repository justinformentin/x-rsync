...
export async function push(options, pullRes) {
    ...
    const nextManifest = await scanLocalDirectory(localRoot, fast, exclude, include);
    const { toUpload, toDelete } = computeDiff(prevManifest, nextManifest, fast);
    logger.info(`Files to upload: ${toUpload.length}, files to delete: ${deleteExtra ? toDelete.length : 0}`);
    if (dry) {
